...
export const StaffEnquiryFormPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: '',
    email: '',
    source: 'walk-in', // Default to walk-in lead
  ...
  // Filter staff list to only show active Sales representatives
  const staffOptions = [
    { value: '', label: 'Select Sales Representative' },
    ...staff
      .filter((s) => s.status === 'active' && s.position === 'sales')
      .map((s: Staff) => ({
        value: s._id,
        label: s.fullName,
      })),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        <Button variant="ghost" onClick={() => navigate('/staff/enquiries')} className="p-2">
          <ArrowLeft size={20} />
        </Button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {isEditing ? 'Edit Walk-in Lead / Enquiry' : 'Add Walk-in Lead Enquiry'}
          </h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            {isEditing
              ? 'Update the details of this prospect lead'
              : 'Manually register a prospect lead visiting the gym and assign follow-up'}
          </p>
        </div>
      </div>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {loadingStaff && (
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">Loading active staff...</p>
              )}
              {staffError && (
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Record comments or conversation details..."
            ...
            <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
              <Button
                type="button"
                variant="ghost"
                onClick={() => navigate('/staff/enquiries')}
                disabled={submitting}
              >
                Cancel
              </Button>
              <Button type="submit" disabled={submitting || loadingStaff}>
                {submitting ? (
                  ...
                  <>
                    ...
                    {isEditing ? 'Update Lead' : 'Log Lead'}
                  </>
                )}
              </Button>
            </div>
...
export default StaffEnquiryFormPage;
