...
export const ExpiringPTCard: React.FC = () => {
  const { expiringPT, loading, fetchExpiringPT } = usePTExpiry();
  const navigate = useNavigate();

  useEffect(() => {
    fetchExpiringPT(7); // Fetch PT packages expiring within 7 days
  }, [fetchExpiringPT]);
  ...
  const expired = expiringPT?.expired || 0;
  const expiringSoon = expiringPT?.expiringSoon || 0;
  const total = expiringPT?.total || 0;
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow duration-200 cursor-pointer overflow-hidden">
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">PT Packages Expiring</h3>
        ...
        {/* View More Button */}
        <button
          onClick={() => navigate('/admin/pt-expiring')}
          className="mt-4 w-full py-2 px-4 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-colors text-sm font-medium"
        >
          View All Expiring PT Packages
        </button>
      </div>
    </div>
  );
